...
numFiles = len([f for f in os.listdir(dropbPath)if os.path.isfile(os.path.join(dropbPath,f))])
...
if tnow.hour < 11 and numFiles > 0:
        ...
        date_for_move(dropbPath,move_d)
...
df.insert(0,'Lkup_Name',lkupName(df,lkup_strings))
df.insert(len(df.columns),'Needs info updated?',needsUpdate(df[df.columns[-5:]],updateCols ,180))
df['contactid']=df['contactid']+'-'+df['Needs info updated?']
...
#print "Saving complete. Query took: " + str(qTime.seconds/60) + ' minutes and '+ str(qTime.seconds%60) + ' seconds.'
